...
%ctor {
    ...
    // Initialize method logging groups
    %init(MethodLogging);
    %init(MethodTracker);
...
// Simple method logging for reverse engineering
static void BHT_LogMethodCall(id self, SEL _cmd) {
    NSLog(@"[BHTwitter] %@ called method: %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd));
}

// Method swizzling helper to log method calls
static void BHT_AddClassMethodLogger(Class targetClass) {
    if (!targetClass) return;
    
    unsigned int methodCount;
    Method *methods = class_copyMethodList(targetClass, &methodCount);
    
    NSLog(@"[BHTwitter] Adding logger to %@ with %d methods", NSStringFromClass(targetClass), methodCount);
    
    for (unsigned int i = 0; i < methodCount; i++) {
        Method method = methods[i];
        SEL selector = method_getName(method);
        
        // Skip property accessors and some common methods to reduce noise
        NSString *selectorName = NSStringFromSelector(selector);
        if ([selectorName hasPrefix:@"_"] || 
            [selectorName hasPrefix:@"set"] ||
            [selectorName isEqualToString:@"dealloc"] ||
            [selectorName isEqualToString:@"description"]) {
            continue;
        }
        
        // Create a dynamic subclass to avoid modifying the original class
        const char *className = [[NSString stringWithFormat:@"BHT_%@", NSStringFromClass(targetClass)] UTF8String];
        Class newClass = objc_allocateClassPair(targetClass, className, 0);
        if (!newClass) {
        ...
        // Add method to the subclass that logs and calls super
        IMP originalImp = method_getImplementation(method);
        const char *typeEncoding = method_getTypeEncoding(method);
        
        // Add a new method implementation that logs and calls the original
        class_addMethod(newClass, selector, imp_implementationWithBlock(^(id self, ...) {
            NSLog(@"[BHTwitter] %@ called %@", NSStringFromClass([self class]), selectorName);
...
// Helper function to add a logger to a specific object
static void BHT_AddLoggerToObject(id object) {
    if (!object) return;
    
    // Create a method swizzling for viewDidAppear: to catch view controllers
    Method method = class_getInstanceMethod([object class], @selector(viewDidAppear:));
    if (method) {
        // This is likely a view controller
        NSLog(@"[BHTwitter] Found a view controller: %@", NSStringFromClass([object class]));
        
        // Add a logger for this specific class
        BHT_AddClassMethodLogger([object class]);
    }
}

// Function to log touch events on a view
static void BHT_LogTouchEventsForView(UIView *view) {
    if (!view) return;
    
    // Add tap gesture recognizer to log touches
    UITapGestureRecognizer *tapRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:nil action:NULL];
    tapRecognizer.cancelsTouchesInView = NO;
    tapRecognizer.delegate = (id<UIGestureRecognizerDelegate>)view;
    
    // Use block to log the touch
    [tapRecognizer addTarget:^(UIGestureRecognizer *recognizer) {
        UIView *touchedView = recognizer.view;
        CGPoint location = [recognizer locationInView:touchedView];
        
        NSLog(@"[BHTwitter] Touch detected on %@ at point: %@", 
              NSStringFromClass([touchedView class]), 
              NSStringFromCGPoint(location));
        
        // Log the responder chain
        UIResponder *responder = touchedView;
        ...
        NSLog(@"[BHTwitter] Responder chain: %@", responderChain);
        
        // Find the target action for this view if possible
        if ([touchedView isKindOfClass:[UIControl class]]) {
            UIControl *control = (UIControl *)touchedView;
            ...
            for (id target in targets) {
                ...
                NSLog(@"[BHTwitter] Control %@ has target: %@ with actions: %@", 
...
// Method logging system for reverse engineering
%hook NSObject

// Hook sendAction:to:from:forEvent: to capture all UI interactions
+ (BOOL)_sendAction:(SEL)action to:(id)target from:(id)sender forEvent:(UIEvent *)event {
    // Only log Twitter-related interactions to reduce noise
    NSString *targetClass = NSStringFromClass([target class]);
    if ([targetClass hasPrefix:@"T1"] || [targetClass hasPrefix:@"TF"]) {
        NSString *senderClass = NSStringFromClass([sender class]);
        NSString *actionName = NSStringFromSelector(action);
        NSLog(@"[BHTwitter] UI Action: %@ from %@ to %@", actionName, senderClass, targetClass);
    }
...
// Hook refresh control to track pull-to-refresh
%hook UIRefreshControl

- (void)beginRefreshing {
    // Find the view controller that owns this refresh control
    UIResponder *responder = self;
    while (responder && ![responder isKindOfClass:[UIViewController class]]) {
        responder = [responder nextResponder];
    }
    ...
    if (responder) {
        ...
        if ([className hasPrefix:@"T1"] || [className hasPrefix:@"TF"]) {
            NSLog(@"[BHTwitter] Refresh started in: %@", className);
        }
    }
...
- (void)endRefreshing {
    ...
    if (responder) {
        NSString *className = NSStringFromClass([responder class]);
        if ([className hasPrefix:@"T1"] || [className hasPrefix:@"TF"]) {
            NSLog(@"[BHTwitter] Refresh ended in: %@", className);
        }
    }
...
// Hook network requests to track API calls
...
- (void)resume {
    ...
    // Only log Twitter API calls
    if ([url containsString:@"twitter.com"] || [url containsString:@"x.com"] || 
        [url containsString:@"api"]) {
        NSLog(@"[BHTwitter] Network request: %@ %@", request.HTTPMethod, url);
    }
...
// Hook timeline controllers
%hook T1TimelineViewController

- (void)viewDidAppear:(BOOL)animated {
    NSLog(@"[BHTwitter] Timeline appeared: %@", NSStringFromClass([self class]));
    %orig;
}

- (void)_reloadTableViewData {
    NSLog(@"[BHTwitter] Timeline reloading data: %@", NSStringFromClass([self class]));
    %orig;
}

%end

// Generic method tracker for Twitter classes
%group MethodTracker
...
%hookf(void, objc_msgSend, id self, SEL _cmd, ...) {
    ...
    if ([className hasPrefix:@"T1"] || [className hasPrefix:@"TF"]) {
        ...
        // Only log methods related to refresh/reload/update
        if ([methodName containsString:@"refresh"] || 
            [methodName containsString:@"reload"] || 
            [methodName containsString:@"update"] ||
            [methodName containsString:@"load"] ||
            [methodName containsString:@"fetch"]) {
            
            NSLog(@"[BHTwitter] %@ called %@", className, methodName);
        }
    }
}
...
// Add this to the existing %ctor function, don't create a new one
// Modify the existing %ctor at line ~4143 to include:
// %init(MethodLogging);
// %init(MethodTracker);