...
- (void)updateTimelineToMatchPreference;
- (void)updateTimelineToMatchPreferenceAnimated:(BOOL)animate;
- (void)toggleTimelineTappedUIButton:(UIButton *)senderButton;
@end
...
static BOOL gAnimateNextTimelineSwitch = NO;

// Keys for UserDefaults
static NSString *const kTimelinePreferenceKey = @"com.thea.twittweak.timelinePreference";

// Get the current timeline preference (Following = YES, For You = NO)
static BOOL GetFollowingTimelinePreference() {
    return [[NSUserDefaults standardUserDefaults] boolForKey:kTimelinePreferenceKey];
}

// Save the current timeline preference
static void SaveTimelinePreference(BOOL isFollowing) {
    [[NSUserDefaults standardUserDefaults] setBool:isFollowing forKey:kTimelinePreferenceKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
    NSLog(@"[TwitTweak] Saved timeline preference: %@", isFollowing ? @"Following" : @"For You");
}

// Function to create and add toggle button to navigation bar
static void AddToggleButtonToNavigationBar(THFHomeTimelineContainerViewController *homeController) {
    if (!homeController) {
        return;
    }

    UINavigationItem *navItem = [homeController navigationItem];

    BOOL isHomeTimeline = [NSStringFromClass([homeController class]) isEqualToString:@"THFHomeTimelineContainerViewController"];
    if (!isHomeTimeline) {
        if (gToggleBarButtonItem && navItem) {
            NSMutableArray *currentRightItems = [navItem.rightBarButtonItems mutableCopy];
            if ([currentRightItems containsObject:gToggleBarButtonItem]) {
                [currentRightItems removeObject:gToggleBarButtonItem];
                navItem.rightBarButtonItems = currentRightItems;
                NSLog(@"[TwitTweak] Removed sparkle toggle button from non-home view: %@", NSStringFromClass([homeController class]));
            }
        }
        return;
    }
    ...
    if (!navItem) {
        NSLog(@"[TwitTweak] Navigation item not available on home view for sparkle button.");
        return;
    }
    ...
    if (gToggleBarButtonItem == nil) { 
        ...
        [customButton addTarget:homeController action:@selector(toggleTimelineTappedUIButton:) forControlEvents:UIControlEventTouchUpInside];
        gToggleBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:customButton];
    } 

    // --- Refined Nav Bar Item Management --- 
    NSMutableArray *currentItems = [navItem.rightBarButtonItems mutableCopy] ?: [NSMutableArray array];
    NSMutableArray *newRightBarButtonItems = [NSMutableArray array];
    BOOL foundOurButton = NO;

    for (UIBarButtonItem *item in currentItems) {
        if (item == gToggleBarButtonItem) {
            if (!foundOurButton) { 
                [newRightBarButtonItems addObject:item];
                foundOurButton = YES;
            }
        } else {
            BOOL isOldOurButton = NO;
            if (item.action == @selector(toggleTimelineTappedUIButton:)) {
                 isOldOurButton = YES;
            }
            if (!isOldOurButton) {
                [newRightBarButtonItems addObject:item];
    ...
    if (!foundOurButton && gToggleBarButtonItem) {
        [newRightBarButtonItems insertObject:gToggleBarButtonItem atIndex:0];
        NSLog(@"[TwitTweak] Adding our toggle button to nav bar.");
    }

    if (![currentItems isEqualToArray:newRightBarButtonItems]) {
        navItem.rightBarButtonItems = newRightBarButtonItems;
        NSLog(@"[TwitTweak] Updated rightBarButtonItems.");
    }
}

// Helper function to check if the segmented controller is part of the home timeline
static BOOL isHomeTimelineSegmentedController(TFNScrollingSegmentedViewController *instance) {
    if (!instance) return NO;
    if ([[instance parentViewController] isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
        return YES;
    }
    UIResponder *responder = instance;
    while ((responder = [responder nextResponder])) {
        if ([responder isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
            return YES;
        }
        if ([responder isKindOfClass:[UIWindow class]] || [responder isKindOfClass:[UIApplication class]]) {
            break;
...
// New helper function to check if a TFNPagingScrollView instance is part of the home timeline
static BOOL isHomeTimelinePagingScrollView(UIView *instance) {
    if (!instance) return NO;
    UIResponder *responder = instance;
    while ((responder = [responder nextResponder])) {
        if ([responder isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
        ...
        // Optimization: if we find a TFNScrollingSegmentedViewController, use the existing helper
        if ([responder isKindOfClass:NSClassFromString(@"TFNScrollingSegmentedViewController")]) {
            if (isHomeTimelineSegmentedController((TFNScrollingSegmentedViewController *)responder)) {
                return YES;
            } else {
                return NO; // It's another segmented controller, so this path is not for home timeline
            }
        }
        if ([responder isKindOfClass:[UIWindow class]] || [responder isKindOfClass:[UIApplication class]]) {
            break;
...
// Hook the segmented view controller to directly modify its behavior
...
- (id)initWithNibName:(NSString *)nibName bundle:(NSBundle *)bundle {
    ...
    if (isHomeTimelineSegmentedController(self)) {
        if ([instance respondsToSelector:@selector(setValue:forKey:)]) {
            @try {
                [instance setValue:@(NO) forKey:@"_animatesSelection"];
                NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: Set _animatesSelection to NO in init.");
            } @catch (NSException *exception) {
                NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: Couldn't set _animatesSelection: %@", exception);
            }
        }
    }
    return instance;
...
// This is the hook for the property setter: - (void)setSelectedIndex:(NSInteger)selectedIndex
- (void)setSelectedIndex:(NSInteger)selectedIndex { // Property hook
    if (isHomeTimelineSegmentedController(self)) {
        BOOL isFollowingPreference = GetFollowingTimelinePreference();
        NSInteger preferredIndex = isFollowingPreference ? 1 : 0;
        NSInteger currentIndex = self.selectedIndex;

        NSLog(@"[TwitTweak] HomeTimelineSegmentedVC -setSelectedIndex (property hook) START. Current: %ld, Attempted: %ld, Preferred: %ld, AnimateNextSwitch_Intent: %@, InitialLoad: %@",
              (long)currentIndex, (long)selectedIndex, (long)preferredIndex, 
              gAnimateNextTimelineSwitch ? @"YES" : @"NO", 
        ...
        if (gAnimateNextTimelineSwitch && !gIsInitialLoadForAnimation) {
            NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: Condition for triggering stock animation met (AnimateNextSwitch=YES, NotInitialLoad=YES).");
            TFNPagingScrollView *pagingScrollView = nil;
            for (UIView *subview in self.view.subviews) {
                if ([subview isKindOfClass:NSClassFromString(@"TFNPagingScrollView")]) {
                    pagingScrollView = (TFNPagingScrollView *)subview;
                    break;
                }
            }
            if (pagingScrollView) {
                CGFloat offsetX = targetIndex * pagingScrollView.frame.size.width;
                if (pagingScrollView.frame.size.width > 0) {
                    NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: Calling setContentOffset animated:YES on PagingScrollView. OffsetX: %f. gAnimateNextTimelineSwitch is currently YES.", offsetX);
                    [pagingScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES]; 
                } else {
                     NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: PagingScrollView width is 0. Forcing update non-animated.");
                     gAnimateNextTimelineSwitch = NO;
                     [self forceUpdateScrollViewToIndex:targetIndex];
                }
        } else {
                 NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: TFNPagingScrollView not found. Forcing update non-animated.");
                 gAnimateNextTimelineSwitch = NO;
                [self forceUpdateScrollViewToIndex:targetIndex];
        }
    } else {
            NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: Condition for triggering stock animation NOT met. Forcing update non-animated to index %ld (InitialLoad: %@, AnimateNextSwitch_Intent: %@).", 
                  (long)targetIndex, gIsInitialLoadForAnimation ? @"YES" : @"NO", gAnimateNextTimelineSwitch ? @"YES" : @"NO");
            gAnimateNextTimelineSwitch = NO;
            [self forceUpdateScrollViewToIndex:targetIndex];
        }
        gAnimateNextTimelineSwitch = NO; // Reset general intent for next setSelectedIndex call unless specifically set again.
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (isHomeTimelineSegmentedController(self)) {
        ...
        NSLog(@"[TwitTweak] HomeTimelineSegmentedVC: scrollViewWillBeginDragging - DRAG PREVENTED, re-forcing to preferredIndex: %ld", (long)preferredIndex);
    } else {
        %orig;
    }
}
...
- (void)forceUpdateScrollViewToIndex:(NSInteger)index {
    ...
    if (scrollViewToUpdate && [scrollViewToUpdate isKindOfClass:[UIScrollView class]]) {
        ...
        if ([actualScrollView respondsToSelector:@selector(_setContentOffsetAnimationDuration:)]) {
            [actualScrollView performSelector:@selector(_setContentOffsetAnimationDuration:) withObject:@(0.0)];
        }
        CGFloat offsetX = index * actualScrollView.frame.size.width;
        if (actualScrollView.frame.size.width > 0) {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (isHomeTimelineSegmentedController(self)) {
        BOOL isFollowing = GetFollowingTimelinePreference();
        NSInteger preferredIndex = isFollowing ? 1 : 0;
        if (self.selectedIndex != preferredIndex) {
...
// Block animation in scroll view
%hook UIScrollView
- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    if (isHomeTimelinePagingScrollView(self)) {
        // This is our specific scroll view. Decide if we allow an animation.
        if (animated && !gIsInitialLoadForAnimation && gAnimateNextTimelineSwitch) {
            // Conditions met for OUR explicit animated switch
            NSLog(@"[TwitTweak] UIScrollView (HomeTimeline): Allowing OUR requested stock animation. (animated:YES, !initialLoad, animateSwitchIntent:YES)");
            %orig(contentOffset, YES); // Attempt stock animation
        } else {
            // All other cases for this scroll view: force non-animated.
            if (animated) { // Log if we are suppressing a requested animation
                 NSLog(@"[TwitTweak] UIScrollView (HomeTimeline): Suppressing UNWANTED/INITIAL animation (requestedAnimated:YES, initialLoad:%@, animateSwitchIntent:%@). Forcing NO.",
                       gIsInitialLoadForAnimation ? @"YES" : @"NO",
                       gAnimateNextTimelineSwitch ? @"YES" : @"NO");
            }
        ...
        %orig; // Normal behavior for other scroll views
    }
}
%end
...
- (void)updateTimelineToMatchPreferenceAnimated:(BOOL)animate {
    ...
    if (segmentedVC) {
        BOOL isFollowing = GetFollowingTimelinePreference();
        NSInteger newIndex = isFollowing ? 1 : 0;

        NSLog(@"[TwitTweak] THFHomeTimelineVC -updateTimelineToMatchPreferenceAnimated: TargetIndex: %ld, AnimateParam: %@, InitialLoad: %@", 
              (long)newIndex, animate ? @"YES" : @"NO", gIsInitialLoadForAnimation ? @"YES" : @"NO");

        gAnimateNextTimelineSwitch = animate; // For the scroll view animation
        segmentedVC.selectedIndex = newIndex;
    } else {
         NSLog(@"[TwitTweak] THFHomeTimelineVC -updateTimelineToMatchPreferenceAnimated: SegmentedVC not found.");
    }
}

%new
- (void)updateTimelineToMatchPreference { // This is the one called by lifecycle methods
    NSLog(@"[TwitTweak] updateTimelineToMatchPreference (defaulting to non-animated)");
    [self updateTimelineToMatchPreferenceAnimated:NO];
}

// This is the NEW button action method
%new
- (void)toggleTimelineTappedUIButton:(UIButton *)senderButton {
    NSLog(@"[TwitTweak] Timeline toggle UIButton tapped.");

    [UIView animateWithDuration:0.1 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        senderButton.transform = CGAffineTransformMakeScale(0.8, 0.8);
        senderButton.alpha = 0.7;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.1 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            senderButton.transform = CGAffineTransformIdentity;
            senderButton.alpha = 1.0;
        } completion:nil];
    }];

    BOOL isFollowing = !GetFollowingTimelinePreference();
    SaveTimelinePreference(isFollowing);
    NSLog(@"[TwitTweak] Toggle button tapped, calling updateTimelineToMatchPreferenceAnimated:YES");
    [self updateTimelineToMatchPreferenceAnimated:YES];
}

%end

// Hook UINavigationBar to detect when it's updated
%hook UINavigationBar
- (void)didMoveToWindow {
    %orig;
    
    // Check if this navigation bar is in the home timeline controller
    UIResponder *nextResponder = self.nextResponder;
    THFHomeTimelineContainerViewController *homeController = nil;
    while (nextResponder) {
        if ([nextResponder isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
            homeController = (THFHomeTimelineContainerViewController *)nextResponder;
            break;
        }
    ...
    if (homeController) {
        // This navigation bar is part of the home timeline controller
        // Add button after a short delay to ensure proper positioning
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            AddToggleButtonToNavigationBar(homeController);
        });
    } else {
        // If not home controller, remove the button if it exists
        if (gToggleBarButtonItem && nextResponder && [nextResponder isKindOfClass:[UIViewController class]]) {
            UIViewController *currentVC = (UIViewController *)nextResponder;
            UINavigationItem *navItem = currentVC.navigationItem;
            if (navItem) {
                ...
                if (rightBarItems.count != navItem.rightBarButtonItems.count) {
                    navItem.rightBarButtonItems = rightBarItems;
                    NSLog(@"[TwitTweak] Removed toggle button from non-home navigation bar");
                }
            }
        }
    }
...
// Prevent the app from overriding our button only on home view
%hook UINavigationItem
- (void)setRightBarButtonItems:(NSArray *)items animated:(BOOL)animated {
    // Make sure our toggle button is included if it exists and we're on home view
    if (gToggleBarButtonItem) {
        // Check if this navigation item belongs to home controller
        BOOL isHomeView = NO;
        // We can't use nextResponder on UINavigationItem, so we need to find the associated view controller
        // Check if this navigation item is part of a view controller's navigation item
        ...
        for (UIWindow *window in windows) {
            ...
            if (rootVC) {
                ...
                while (queue.count > 0) {
                    UIViewController *vc = [queue objectAtIndex:0];
                    [queue removeObjectAtIndex:0];
                    
                    if (vc.navigationItem == self && [vc isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
                        isHomeView = YES;
                        break;
                    }
        ...
        if (isHomeView) {
            ...
            // Check if our button is already in the array
            ...
            // Add our button if it's not already there
            if (!buttonExists) {
                [newItems insertObject:gToggleBarButtonItem atIndex:0];
            }
            
            %orig(newItems, animated);
        } else {
            // For non-home views, ensure our button is not included
            NSMutableArray *newItems = [NSMutableArray array];
            if (items) {
                for (UIBarButtonItem *item in items) {
                    if (item != gToggleBarButtonItem) {
            ...
            %orig(newItems, animated);
            NSLog(@"[TwitTweak] Ensured toggle button is not on non-home view");
        }
    } else {
        %orig;
    }
...
// Hook to hide the horizontal label collection view (tab bar) only on home view
%hook TFNScrollingHorizontalLabelCollectionView
- (void)didMoveToWindow {
    %orig;
    
    // Check if parent is home timeline controller
    UIResponder *nextResponder = self.nextResponder;
    BOOL isHomeView = NO;
    while (nextResponder) {
        if ([nextResponder isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
    ...
    if (isHomeView) {
        // Attempt to remove from superview
        if (self.superview) {
            [self removeFromSuperview];
            NSLog(@"[TwitTweak] Removed TFNScrollingHorizontalLabelCollectionView from superview on home view");
        } else {
            // If removal fails, hide and move off-screen
            self.hidden = YES;
            self.alpha = 0;
            self.userInteractionEnabled = NO;
            self.frame = CGRectMake(-1000, -1000, 0, 0);
            NSLog(@"[TwitTweak] Could not remove, so hid and moved TFNScrollingHorizontalLabelCollectionView off-screen on home view (didMoveToWindow)");
        }
        
        // Ensure it stays hidden over time
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (self.superview) {
                [self removeFromSuperview];
                NSLog(@"[TwitTweak] Re-removed (1) TFNScrollingHorizontalLabelCollectionView on home view");
            } else {
                self.hidden = YES;
                self.alpha = 0;
                self.userInteractionEnabled = NO;
        ...
        // Ensure it's visible on other views
        self.hidden = NO;
        self.alpha = 1.0;
        self.userInteractionEnabled = YES;
        NSLog(@"[TwitTweak] Showing TFNScrollingHorizontalLabelCollectionView on non-home view");
    }
}
...
- (void)willMoveToWindow:(UIWindow *)newWindow {
    ...
    if (isHomeView) {
        ...
        self.alpha = 0;
        self.userInteractionEnabled = NO;
        NSLog(@"[TwitTweak] Hiding TFNScrollingHorizontalLabelCollectionView on home view (willMoveToWindow)");
    }
}
...
- (void)didMoveToSuperview {
    ...
    if (isHomeView) {
        ...
        self.alpha = 0;
        self.userInteractionEnabled = NO;
        NSLog(@"[TwitTweak] Hiding TFNScrollingHorizontalLabelCollectionView on home view (didMoveToSuperview)");
    }
}
...
- (void)setHidden:(BOOL)hidden {
    ...
    if (isHomeView) {
        %orig(YES); // Force hidden on home view
        self.alpha = 0;
        self.userInteractionEnabled = NO;
        NSLog(@"[TwitTweak] Forcing TFNScrollingHorizontalLabelCollectionView to stay hidden on home view");
    } else {
        %orig(hidden); // Normal behavior on other views
    }
}
%end
    ...
    if (isHomeTimelinePagingScrollView(self)) {
        ...
        if (self.gestureRecognizers) {
            for (UIGestureRecognizer *recognizer in self.gestureRecognizers) {
                if ([recognizer isKindOfClass:[UIPanGestureRecognizer class]] && recognizer.isEnabled) {
                    recognizer.enabled = NO;
                }
            }
        }
        // After frame is set and properties are fixed, ensure content is visible
        UIResponder *responder = self;
        TFNScrollingSegmentedViewController *segmentedVC = nil;
        while ((responder = [responder nextResponder])) {
            if ([responder isKindOfClass:NSClassFromString(@"TFNScrollingSegmentedViewController")]) {
                segmentedVC = (TFNScrollingSegmentedViewController *)responder;
                break;
            }
            if ([responder isKindOfClass:[UIViewController class]] && ![responder isKindOfClass:NSClassFromString(@"THFHomeTimelineContainerViewController")]) {
                break; 
            }
        }
        if (segmentedVC && isHomeTimelineSegmentedController(segmentedVC)) {
...
- (void)setContentSize:(CGSize)contentSize {
    // For home timeline, allow original contentSize. We only disable user interaction.
    if (isHomeTimelinePagingScrollView(self)) {
        %orig(contentSize);
    } else {
        %orig(contentSize);
